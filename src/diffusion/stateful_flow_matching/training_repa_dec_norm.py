...
def _rms_normalize_tokens(tokens: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """Per-sample RMS normalization for direct DINO feature targets."""
    rms = torch.sqrt(tokens.float().pow(2).mean(dim=(1, 2), keepdim=True) + eps)
    return tokens / rms.to(dtype=tokens.dtype)

def _normalize_tokens(tokens: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """Per-sample mean-centered normalization for direct DINO feature targets."""
    tokens_f = tokens.float()
    tokens_f = tokens_f - tokens_f.mean(dim=(1, 2), keepdim=True)
    norm = torch.sqrt(tokens_f.pow(2).mean(dim=(1, 2), keepdim=True) + eps)
    return (tokens_f / norm).to(dtype=tokens.dtype)
...
class REPATrainer(BaseTrainer):
    def __init__(
        ...
        # self.proj1 = MLP(proj_denoiser_dim, [proj_hidden_dim, proj_hidden_dim, proj_dim], norm_layer=nn.BatchNorm1d)
        # self.sigreg = sigreg()
        # self.proj1 = nn.Conv2d(proj_denoiser_dim, proj_dim, kernel_size=3, padding=1)
    ...
    def _impl_trainstep(self, net, ema_net, raw_images, x, y, step):
        ...
        if self.use_dog_bank_align:
            ...
            with torch.no_grad():
                ...
                # enc_target = _normalize_tokens(
                #     _resize_tokens(dino_tokens, dino_hw, enc_hw),
                #     eps=self.dog_bank_eps,
                # ).detach()

                dec_target = _normalize_tokens(
                    _resize_tokens(dino_de_tokens, dino_hw, dec_hw),
                    eps=self.dog_bank_eps,
                ).detach()
