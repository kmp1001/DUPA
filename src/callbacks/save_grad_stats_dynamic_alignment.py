...
class SaveGradStatsHook(Callback):
    """
    训练时：
      每隔 log_interval 把 step / fm_loss / alignment loss / gate entropy
      以及各 frequency band gate/loss 指标追加写入 CSV:

          {default_root_dir}/{save_dir}/{filename}

    验证开始时：
      读取截至当前为止的全部日志，
      画出 loss / gate / band loss 曲线并保存到:
    ...
    def _sync_bank_names(self, pl_module=None, payload=None):
        ...
        if isinstance(payload, dict):
            enc_names.extend(self._names_from_payload(payload, "enc_gate_"))
            enc_names.extend(self._names_from_payload(payload, "enc_band_loss_"))
            dec_names.extend(self._names_from_payload(payload, "dec_gate_"))
    ...
    def _names_from_payload(self, payload, prefix):
        return [
            ...
            if (
                ...
                and key not in {"enc_gate_entropy", "dec_gate_entropy"}
            )
        ]

    def _extra_log_fields(self):
        """
        这里集中定义 alignment / gate 相关字段。
        ...
        fields.extend([f"enc_gate_{name}" for name in self._enc_bank_names])
        fields.extend([f"enc_band_loss_{name}" for name in self._enc_bank_names])
        fields.extend([f"dec_gate_{name}" for name in self._dec_bank_names])
    ...
    def _save_series_plot(self, plot_dir, filename_base, title, ylabel, steps, series_items, trainer):
        series_items = [
            (label, values)
    ...
    def _save_metric_plots(self, trainer: "pl.Trainer", steps, series):
        ...
        saved_paths = []
        for field in ["enc_gate_entropy", "dec_gate_entropy", "gate_entropy_scale"]:
            latest_path, step_path = self._save_series_plot(
                ...
                ylabel="value",
                steps=steps,
                series_items=[(field, series.get(field, []))],
                trainer=trainer,
            )
        ...
        for prefix, filename_base in prefix_groups:
            fields = [
                ...
                if (
                    ...
                    and field not in {"enc_gate_entropy", "dec_gate_entropy"}
                )
            ]
    ...
    def on_validation_start(
        ...
        if saved_paths:
            rank_zero_info(
                f"Saved {len(saved_paths)} gate/band metric curve groups to {self._plot_dir(trainer)}"
            )